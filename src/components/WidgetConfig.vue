  ...
  <div v-if="show">
    <Form label-position="top">
      ...
      <FormItem label="宽度" v-if="Object.keys(data.options).indexOf('width')>=0">
        <Input v-model="data.options.width"/>
      </FormItem>

      <FormItem label="大小" v-if="Object.keys(data.options).indexOf('size')>=0">
        宽度：<Input style="width: 90px;" number v-model.number="data.options.size.width"></Input>
        高度：<Input style="width: 90px;" number v-model.number="data.options.size.height"></Input>
      </FormItem>
      
      <FormItem label="占位内容" v-if="Object.keys(data.options).indexOf('placeholder')>=0 && (data.type!='time' || data.type!='date')">
        <Input v-model="data.options.placeholder"></Input>
      </FormItem>
      <FormItem label="布局方式" v-if="Object.keys(data.options).indexOf('inline')>=0">
        <RadioGroup v-model="data.options.inline" type="button">
      ...
      <FormItem label="显示输入框" v-if="Object.keys(data.options).indexOf('showInput')>=0">
        <i-switch v-model="data.options.showInput" ></i-switch>
      </FormItem>
      ...
      <FormItem label="是否多选" v-if="data.type=='select'">
        <i-switch v-model="data.options.multiple" @change="handleSelectMuliple"></i-switch>
      </FormItem>
      ...
      <FormItem label="选项" v-if="Object.keys(data.options).indexOf('options')>=0">
        <RadioGroup v-model="data.options.remote" size="small" type="button" style="margin-bottom:10px;">
          <Radio :label="'false'">静态数据</Radio>
          <Radio :label="'true'">远端数据</Radio>
        </RadioGroup>
        <template v-if="data.options.remote!=='false'">
          <div>
            <Input size="small" style="display: inline-table;" v-model="data.options.remoteFunc">
              <template slot="prepend">远端方法</template>
            </Input>
            <Input size="small" style="display: inline-table;" v-model="data.options.props.value">
              <template slot="prepend">值</template>
            </Input>
            <Input size="small" style="display: inline-table;" v-model="data.options.props.label">
        ...
        <template v-else>
          <template v-if="data.type=='radio' || (data.type=='select'&&!data.options.multiple)">
            <RadioGroup v-model="data.options.defaultValue">
              <draggable element="ul" style="overflow: hidden;"  :list="data.options.options" 
              ...
              >
                <li v-for="(item, index) in data.options.options" :key="index" >
                  ...
                  >
                    <Input :style="{'width': data.options.showLabel? '90px': '190px' }" size="small" v-model="item.value"></Input>
                    <Input style="width:100px;" size="small" v-if="data.options.showLabel" v-model="item.label"></Input>
                    <!-- <input v-model="item.value"/> -->
                  </Radio>
                  <i class="drag-item" style="font-size: 16px;margin: 0 5px;cursor: move;"><icon name="bars" ></icon></i>
                  <Button @click="handleOptionsRemove(index)" shape="circle" icon="ios-trash" size="small"></Button>
                </li>
              </draggable>
          ...
          <template v-if="data.type=='checkbox' || (data.type=='select' && data.options.multiple)">
            <Checkbox-group v-model="data.options.defaultValue">
      ...
      <FormItem label="默认值" v-if="Object.keys(data.options).indexOf('defaultValue')>=0 && (data.type == 'textarea' || data.type == 'input' || data.type=='rate' || data.type=='color' || data.type=='switch')">
        <Input v-if="data.type=='textarea'" type="textarea" :rows="5" v-model="data.options.defaultValue"></Input>
        <Input v-if="data.type=='input'" v-model="data.options.defaultValue"></Input>
        <Rate v-if="data.type == 'rate'" style="display:inline-block;vertical-align: middle;" :max="data.options.max" :allow-half="data.options.allowHalf" v-model="data.options.defaultValue"></Rate>
        <Button type="text" v-if="data.type == 'rate'" style="display:inline-block;vertical-align: middle;margin-left: 10px;" @click="data.options.defaultValue=0">清空</Button>
        <ColorPicker 
          v-if="data.type == 'color'"
          v-model="data.options.defaultValue"
          :show-alpha="data.options.showAlpha"
        ></ColorPicker>
        <i-switch v-if="data.type=='switch'" v-model="data.options.defaultValue"></i-switch>
      </FormItem>
      ...
      <template v-if="data.type == 'time' || data.type == 'date'">
        ...
        <FormItem label="占位内容" v-if="(!data.options.isRange && data.type == 'time') || (data.type != 'time' && data.options.type != 'datetimerange' && data.options.type != 'daterange')">
          <Input v-model="data.options.placeholder"></Input>
        </FormItem>
        <FormItem label="开始时间占位内容" v-if="(data.options.isRange) || data.options.type=='datetimerange' || data.options.type=='daterange'">
          <Input v-model="data.options.startPlaceholder"></Input>
        </FormItem>
        <FormItem label="结束时间占位内容" v-if="data.options.isRange || data.options.type=='datetimerange' || data.options.type=='daterange'">
          <Input v-model="data.options.endPlaceholder"></Input>
        </FormItem>
        <FormItem label="格式">
          <Input v-model="data.options.format"></Input>
        </FormItem>
        <FormItem label="默认值" v-if="data.type=='time' && Object.keys(data.options).indexOf('isRange')>=0">
          <TimePicker  
            key="1"
            style="width: 100%;"
      ...
      <template v-if="data.type=='imgupload'">
        <FormItem label="最大上传数">
          <Input number v-model.number="data.options.length"></Input>
        </FormItem>
      ...
      <template v-if="data.type == 'grid'">
        <FormItem label="栅格间隔">
          <Input number v-model.number="data.options.gutter"></Input>
        </FormItem>
        <FormItem label="列配置项">
          <draggable element="ul" style="overflow: hidden;" :list="data.columns" 
            :options="{group:{ name:'options'}, ghostClass: 'ghost',handle: '.drag-item'}"
          >
            <li v-for="(item, index) in data.columns" :key="index" >
              <i class="drag-item" style="font-size: 16px;margin: 0 5px;cursor: move;"><icon name="bars" ></icon></i>
              <Input placeholder="栅格值" size="small" style="width: 100px;" number v-model.number="item.span"></Input>
      ...
      <template v-if="data.type != 'grid'">
        ...
        <FormItem label="操作属性">
          <Checkbox v-model="data.options.readonly" v-if="Object.keys(data.options).indexOf('readonly')>=0">完全只读</Checkbox>
          <Checkbox v-model="data.options.disabled" v-if="Object.keys(data.options).indexOf('disabled')>=0">禁用	</Checkbox>
          <Checkbox v-model="data.options.editable" v-if="Object.keys(data.options).indexOf('editable')>=0">文本框可输入</Checkbox>
          <Checkbox v-model="data.options.clearable" v-if="Object.keys(data.options).indexOf('clearable')>=0">显示清除按钮</Checkbox>
          <Checkbox v-model="data.options.arrowControl" v-if="Object.keys(data.options).indexOf('arrowControl')>=0">使用箭头进行时间选择</Checkbox>
        </FormItem>
        <FormItem label="校验">
          <div>
            <Checkbox v-model="data.options.required">必填</Checkbox>
          </div>
          <Select v-if="Object.keys(data.options).indexOf('dataType')>=0" v-model="data.options.dataType" size="small" >
          ...
          <div v-if="Object.keys(data.options).indexOf('pattern')>=0">
            <Input size="small" v-model.lazy="data.options.pattern"  style=" width: 240px;" placeholder="填写正则表达式"></Input>
          </div>
        </FormItem>
      </template>
    </Form>
